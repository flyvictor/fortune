...
function route(
  ...
  const sendResponse = async function (req, res, status, object) {
    ...
    try {
      ...
      if (req.linker) {
        ...
        const body = await appendLinks.call(_this, object, req, res);
        object = await booster.mergeResults(req, req.linker, body);
      } else {
        object = await localTracer('linking-resources', () => {
          zipkinTracer.recordBinary('fortune.include_paths', req.query.include);
          return appendLinks.call(_this, object, req, res);
        });
      }

      await localTracer('metadata-providers', async () => {
        const includeMeta =
          req.query.includeMeta &&
          !_.isBoolean(req.query.includeMeta) &&
          _.without(ensureQueryArray(req.query.includeMeta), 'count', 'true');
        if (!includeMeta || !includeMeta.length) return;
        object.meta = object.meta || {};
        await Promise.all(
          _.map(includeMeta, async function (metaKey) {
            if (!metadataProviders[metaKey]) {
              object.meta[metaKey] = 'Metadata provider is not defined';
              return;
            }
            object.meta[metaKey] = await Promise.resolve(
              metadataProviders[metaKey].call(object, req, res),
            );
          }),
        );
      });

      const transform = await localTracer('before-response-send-hook', () =>
        ...
        if (req.zipkinTraceId) {
          zipkinTracer.scoped(() => {
            ...
            zipkinTracer.recordBinary(
              ...
              transformedObject[collection].length,
            );
            zipkinTracer.recordAnnotation(new Annotation.ServerSend());
          });
        }
      ...
      if (!res.headersSent && !res._headerSent) {
        await sendError(req, res, 500, err);
      }
    }
  };
  ...
  async function createResource(req, res) {
    ...
    if (req.body.linked) {
      try {
        ...
        const linkedBefores = {};
        for (const [key, linkedResources] of Object.entries(
          omitExternal(req.body.linked),
        )) {
        ...
        const transformed = await resolveObjectValues({
          mainResources: mainTransform,
          linkedResources: resolveObjectValues(
            // gets indexed by resource name
            _.reduce(
              linkedBefores,
        ...
        if (hooksFailed(_.flatten(_.values(transformed.linkedResources), true)))
          return;
        ...
        const mappedMainResources = await Promise.all(
          _.map(matchedMainResources, function (r) {
            return resolveObjectValues(r);
          }),
        );

        const createdMainResources = await createResources(
          model,
          mappedMainResources,
        );
        ...
        await Promise.all(
          Object.keys(transformed.linkedResources).map((key) => {
            const linkedResourcesList = _.isArray(
              transformed.linkedResources[key],
            )
              ? transformed.linkedResources[key]
              : [transformed.linkedResources[key]];
            return applyAfterHooks(
        ...
        const afterTransformedMainResources = await applyAfterHooks(
          model.modelName || model,
          createdMainResources,
          req,
          res,
        );
      ...
      try {
        ...
        const afterTransformed = await applyAfterHooks(
          model.modelName || model,
          createdResources,
          req,
          res,
    ...
    function mix(main, linked, linkedModel) {
      ...
      return Promise.all(
        _.map(_.isArray(main) ? main : [main], function (item) {
          ...
          toCreate.$$createPromise = createResources(linkedModel, [
            _.omit(toCreate, 'id'),
          ]).then((r) => {
            toCreate.id = r[0].id;
            return r[0].id;
    ...
    localTracer('before-read-hooks', () => beforeReadHook({}, req, res))
      .then(
        instrumentor.createTracer(`${tracePrefix} parse`, function () {
          ...
          if (req.query.sort) {
            options.sort = parseQuerySort(req.query.sort);
          } else if (model.schema.options.defaultSort) {
            options.sort = model.schema.options.defaultSort;
          }
          if (req.query.page) {
          ...
          if (!_.isNumber(options.limit)) {
            options.limit = model.schema.options.defaultLimit;
          }

          const readPreference = parseQueryReadPreference(
            req.query.readPreference,
          );
          if (readPreference) {
            options.readPreference = readPreference;
          }
          options.canUseShardedCollection = !!req.query.canUseShardedCollection;
            ...
            instrumentor.createTracer(`${tracePrefix} read`, function (query) {
              ...
              queries.resources = localTracer('reading-database', () => {
                ...
                zipkinTracer.recordBinary(
                  ...
                  zipkin.safeStringify(options),
                );
                return adapter.findMany(model, query, options);
              });
              ...
              if (
                ...
                if (process.env.DISASTER_RECOVERY_COUNT_ENABLED) {
                  ...
                  queries.count = Promise.resolve(1);
                }

                if (req.query.filter) {
                  queries.filterCount = localTracer(
                    'counting-filter-resources',
                    () => adapter.count(model, query, options),
                  );
                }
              }
              return resolveObjectValues(queries);
            }),
          );
        }),
      )
  ...
  function getResource(req, res) {
    ...
    const readPreference = parseQueryReadPreference(req.query.readPreference);
    if (readPreference) {
      options.readPreference = readPreference;
    }
    ...
    findResourcesWithFortuneExtensions(ids, options, localTracer, req, res)
      // run after read hook
      .then(
        function (resources) {
          if (!resources) return;
  ...
  async function callAction(req, res) {
    ...
    try {
      const response = await _this.direct.get(collection, req);
      const docs = response.body && response.body[collection];
      // direct.get may resolve with an error response body for forbidden/missing resources.
      // Forward that response instead of treating it as an empty action result.
      if (!docs) return forwardDirectResponse(response, req, res);
  ...
  async function callGenericAction(req, res) {
    ...
    try {
      const result = await _this.actions.handleAction(
        params,
        req,
        res,
        _this.adapter,
  ...
  function forwardDirectResponse(response, req, res) {
    if (response.body && response.body.error) {
      res.status(response.statusCode || 500);
      res.set('Content-Type', 'application/json');
      const str = production
        ? JSON.stringify(response.body, null, null)
  ...
  function isActionHandled(result) {
    // Action handlers return undefined/false when they have already written to res.
    // Any returned document should be serialized by the action route.
    if (_.isArray(result)) {
      return _.every(result, function (item) {
        return _.isUndefined(item) || item === false;
      });
    }
    return _.isUndefined(result) || result === false;
  }
  ...
  async function getLinked(ids, type, req) {
    if (ids.length > 0) {
    ...
    return type ? { type: inflect.pluralize(type) } : undefined;
  }
  ...
  async function appendLinked(
    ...
    await fetchChildren(linkpath, includePaths, resources, schema);
    return body;

    function fetchChildren(linkpath, config, resources, schema) {
      return Promise.all(
        _.map(_.keys(config), async function (key) {
          ...
          const result = await getLinked.call(_this, ids, type, req, res);
  ...
  async function findResourcesWithFortuneExtensions(
    ...
    try {
      ...
      const projection = _.clone(options);
      projection.canUseShardedCollection = !!req.query.canUseShardedCollection;
      ...
      const projectionWithDeleted = _.clone(options);
...
const parseQuerySort = (sort) => {
  if (!sort || _.isString(sort) || !_.isObject(sort)) {
    return sort;
  }
...
const parseQueryReadPreference = (readPreference) => {
  if (
    !readPreference ||
    ![
      'primary',
