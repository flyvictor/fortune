      ...
      return RSVP.all(before)
        .then(function(resources) {
          if (_.some(resources, function(res) {
            ...
            console.log('Not creating resource due to before transform returning false');
            return;
          }else{
            return resources;
          }
        ...
        if (_.size(ids) > 0) {
          ...
          findPromise = beforeReadHook(relatedModel, {}, req, res)
            .then(function(){
              ...
              return adapter.findMany(relatedModel, req.query.filter, projection.limit ? projection.limit : undefined);
            }, function(err){
              sendError(req, res, 500, err);
            });
        } else {
