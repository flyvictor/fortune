...
function route(name, model, resources, inflect, querytree, metadataProviders, zipkinTracer) {
 ...
 router.get(collectionRoute, getResources);
  ...
  function getResource(req, res) {
    ...
    // Issue 156: Check to see if include is requested in field, if not then append to get linked resources.
    var fields = req.query.fields ? req.query.fields.split(',') : [];
    var includes = req.query.include ? req.query.include.split(',') : [];
    var selectProjectionParam;
    if(fields === includes){
      selectProjectionParam = fields.join(',');
    }
    else {
      selectProjectionParam = fields.concat(includes).join(',');
    }

    var select = parseSelectProjection(selectProjectionParam, inflect.pluralize(model.modelName));
  ...
  function deleteResource(req, res) {
    ...
    localTracer('reading-for-validation', () => adapter.findMany(model, id, projection))
      .then(function(resources) {
  ...
  function updateResource(req, res) {
    ...
    zipkinTracer.scoped(() => {
      ...
      return zipkinTracer.local('reading-resource-for-validation', () => {
        return adapter.find(model, id, project)
      });
    })
  ...
  function appendLinked(linkpath, body, resources, schema, inclusions, req, res) {
    ...
    return fetchChildren(linkpath, includePaths, resources, schema).then(function() {
      ...
      // issue 156: remove the links from top level resource as it was not requested.
