...
function route(name, model, resources, inflect, querytree, metadataProviders, zipkinTracer) {
  ...
  function Hook(time, type){
    return function(model, resource, request, response, isNew){
      ...
      //  zipkinTracer.setId(request.zipkinTraceId);
      //  return zipkinTracer.local(`${time}-${type}-hooks`, () => {
                ...
                if (transform) {
                  if (transform.then) {
                    transform = transform.then(function (data) {
                      ...
                      //return zipkinTracer.scoped(() => {
                      //  zipkinTracer.setId(request.zipkinTraceId);
                      //  return zipkinTracer.local(`${time}-${type}-hook-${h.name}}`, () => {
                      //    zipkinTracer.recordBinary('hook-resource-id', data && data.id && data.id.toString());
                    ...
                    /*
                    transform = zipkinTracer.scoped(() => {
                      zipkinTracer.setId(request.zipkinTraceId);
                      return zipkinTracer.local(`${time}-${type}-hook-${h.name}`, () => {
                        zipkinTracer.recordBinary('hook-resource-id', transform && transform.id && transform.id.toString());
                        return fn.call(transform, request, response);
                      })
                    });
                    */
                  }
  ...
  function createResource(req, res){
    ...
    function createResources(model, resources) {
      ...
      //console.log('starting db write with trace id ', req.zipkinTraceId._spanId);
      //  console.log("still within trace id ", req.zipkinTraceId._spanId);
