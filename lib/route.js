  ...
  var sendResponse = function (req, res, status, object) {
    ...
    linked
      .then(function(object){
        var includeMeta = req.query.includeMeta && _.without(req.query.includeMeta.split(','), 'count', 'true');
    ...
    beforeReadHook({}, req, res)
      .then(function(){
        ...
        return querytree.parse(model.modelName, _.extend(match, req.query.filter)).then(function(query){
          ...
          if(req.query.includeMeta && (req.query.includeMeta.split(',').indexOf('count') !== -1 || req.query.includeMeta.split(',').indexOf('true') !== -1)) {
      ...
      req.body.forEach(function (operation) {
        ...
        if (operation.op === 'replace') {
          if (field.length > 1 && field[0] !== "links") {
            ...
            path.$unset[field[field.length-1]] = 1;
          } else {
            path.$set = path.$set || {};
