 ...
 */
function route (name, model, resources, inflect, querytree, metadataProviders, zipkinTracer) {
  ...
  const sendError = function (req, res, status, error) {
    if (error) {
      console.error('fortune sendError:');
      console.error(error);
    }
    const object = {
    ...
    return Promise.resolve(function () {
      return beforeErrorResponseHook(object, req, res);
    ...
    linked
      .then(function (object){
        return localTracer('metadata-providers', () => {
          const includeMeta = req.query.includeMeta && !_.isBoolean(req.query.includeMeta) && _.without(req.query.includeMeta.split(','), 'count', 'true');
          ...
          return RSVP.all(_.map(includeMeta, function (metaKey) {
            ...
            const promise = tmp.then ? tmp : new RSVP.Promise(function (r) {
              r(tmp);
            });
      ...
      .then(function (object) {
        localTracer('before-response-send-hook', () => beforeResponseHook(object, req, res))
          .then(function (transform){
          ...
          try {
            if (req.zipkinTraceId) {
              zipkinTracer.scoped(() => {
                ...
                zipkinTracer.recordBinary('fortune.response_resources_count', object[collection].length);
                zipkinTracer.recordAnnotation(new Annotation.ServerSend());
              });
            }
          }catch(e){
  ...
  function Hook (time, type){
    return function (model, resource, request, response, isNew){
      ...
      const tracePrefix = `${instrumentorObj.options.tracePrefix + model } - ${ time }-hooks`;
  ...
  function createResource (req, res){
    zipkinTracer.scoped(() => {
      ...
      zipkinTracer.recordBinary('fortune.resources_count', req.body[collection].length);
      zipkinTracer.recordBinary('fortune.is_compound_post', _.isObject(req.body.linked).toString());
    ...
    if (req.body.linked){
      ...
      runBefores(model.modelName || model, mainResources)
        ...
        .then(function (transformed){
          ...
          const refsFromMain = getAssociations.call(_this, _this._schema[inflect.singularize(collection)]);
          ...
          _.each(transformed.linkedResources, function (linkedResources, key) {
            const singularKey = options.inflect ? inflect.singularize(key) : key;
    ...
    function terminateOnRejectedHooks (resources){
      if (_.some(resources, function (res) {
        ...
        console.log('Terminating request due to a transform returning false/empty');
        return;
      }else{
        return resources;
      }
    ...
    function mix (main, linked, linkedModel){
      ...
      return RSVP.all(_.map(_.isArray(main) ? main : [main], function (item){
        ...
        toCreate.$$createPromise = createLinked(linkedModel, _.omit(toCreate, 'id'));
    ...
    function createResources (model, resources) {
      ...
      return localTracer('database-write', () => {
 ...
 router.get(collectionRoute, getResources);
  ...
  function getResources (req, res) {
    zipkinTracer.scoped(() => {
      ...
      zipkinTracer.recordBinary('fortune.resource', name);
      zipkinTracer.recordBinary('fortune.client_query', zipkin.safeStringify(req.query));
      zipkinTracer.recordAnnotation(new Annotation.ServerRecv());
    });
    const localTracer = zipkin.makeLocalCallsWrapper(req, zipkinTracer);
    ...
    const match = {};
    let projection = {};
    const select = parseSelectProjection(req.query.fields, inflect.pluralize(model.modelName));
    ...
    localTracer('before-read-hooks', () => beforeReadHook({}, req, res))
      .then( instrumentor.createTracer( `${tracePrefix } parse`, function (){
        ...
        return localTracer('resolving-cross-resource-query', () => querytree.parse(req, model.modelName, _.extend(match, req.query.filter)))
          .then( instrumentor.createTracer( `${tracePrefix } read`, function (query){
            const queries = {};
            queries.resources = localTracer('reading-database', () => {
              zipkinTracer.recordBinary('fortune.mongo_query', zipkin.safeStringify(query));
            ...
            if(req.query.includeMeta && (_.isBoolean(req.query.includeMeta) || req.query.includeMeta.split(',').indexOf('count') !== -1 || req.query.includeMeta.split(',').indexOf('true') !== -1)) {
              ...
              if (process.env.DISASTER_RECOVERY_COUNT_ENABLED){
                queries.count = localTracer('counting-all-resources', () => adapter.count(model, { $and: req.query.fortuneExtensions }, projection));
              }else{
                queries.count = new RSVP.resolve(1);
              }

              if (req.query.filter) {
                queries.filterCount = localTracer('counting-filter-resources', () => adapter.count(model, query, projection));
              }
            }
      ...
      .then( instrumentor.createTracer( `${tracePrefix } sending-response`, function (r) {
        ...
        const status = error.constructor.name === 'CastError' || error.message.match(/^Cast/) ? 400 : 500;
  ...
  function replaceResource (req, res) {
    ...
    localTracer('reading-for-validation', () => adapter.find(model, id))
      .then(function () {
        // do before write hook
        return localTracer('before-write-hooks', () => beforeWriteHook(update, req, res))

        // update the resource
          .then(function (update) {
            if (!update) return;
            return localTracer('writing-database', () => {
              const match = adapter.preupdate(model, id);
        ...
        // do after transform
          .then(function (update) {
            if (!update) return;
            return localTracer('after-write-hooks', () => afterWriteHook(update, req, res));
          }, function (error) {
            sendError(req, res, 500, error);
          })
        ...
        // send the response
          .then(function (update) {
            ...
            function () {
              ...
              // do before transform
              localTracer('before-write-hooks', () => beforeWriteHook(update, req, res, null, { __isNew: true }))

              // create the resource
                .then(function (resource) {
                  return localTracer('writing-database', () => adapter.create(model, id, resource));
                }, function (error) {
                  sendError(req, res, 403, error);
                })
              ...
              // do after transform
                .then(function (resource) {
                  return localTracer('after-write-hooks', () => afterWriteHook(resource, req, res));
                }, function (error) {
                  sendError(req, res, 500, error);
                })
  ...
  function deleteResource (req, res) {
    ...
    localTracer('reading-for-validation', () => findResourcesWithFortuneExtensions(ids, projection, localTracer, req, res))
      .then(function (resources) {
        if (!resources) return;
    ...
    // let's delete it (or only mark deleted by )
      .then(function (resources) {
        if (_.isUndefined(resources) || _.some(resources, function (r){return !r;})) return;
        return localTracer('writing-database', () => {
          return destroy ? adapter.delete(model, ids) : adapter.markDeleted(model, ids);
        });
  ...
  function updateResource (req, res) {
    zipkinTracer.scoped(() => {
      ...
      zipkinTracer.recordBinary('fortune.resource', name);
      zipkinTracer.recordBinary('fortune.update_operations_count', req.body.length);
      zipkinTracer.recordAnnotation(new Annotation.ServerRecv());
    });
    ...
    zipkinTracer.scoped(() => {
      ...
      return zipkinTracer.local('reading-resource-for-validation', () => {
        const localTracer = zipkin.makeLocalCallsWrapper(req, zipkinTracer);
        return findResourcesWithFortuneExtensions([id], project, localTracer, req, res);
      });
    })
    .then(function ( returnedDocuments ){
      if (!returnedDocuments) return;
      return zipkinTracer.scoped(() => {
        ...
        try {
          ...
          updates = routeHelpers.buildPatchOperations(model, returnedDocuments[0], req.body);
        } catch (error) {
          console.log(error);
          return sendError(req, res, 400, error);
        }
    ...
    .then(function (updates) {
      if (!updates) return;
      let chain = RSVP.resolve();
      updates.forEach(function (update){
        chain = chain.then(function (prev){
          return zipkinTracer.scoped(() => {
            zipkinTracer.setId(req.zipkinTraceId);
            return zipkinTracer.local('writing-database', () => {
              const match = adapter.preupdate(model, id);
    ...
    function terminateOnRejectedHooks (updates){
      if (_.some(updates, function (update) {
        ...
        console.log('Terminating PATCH request due to a transform returning false/empty');
        return;
      }else{
        return updates;
      }
  ...
  function linkedIds (resources, path, schema) {
    ...
    _.each(resources, function (resource) {
      if (_.isArray(schema[path]) || _.isObject(schema[path])) {
        const isExt = (_.isArray(schema[path]) ? schema[path][0] : schema[path]).external;
    ...
    function fetchChildren (linkpath, config, resources, schema) {
      return RSVP.all(_.map(_.keys(config), function (key) {
        ...
        return getLinked.call(_this, ids, type, req, res).then(function (result) {
          const relation = _.isArray(schema[key]) ? schema[key][0] : schema[key];
    ...
    _.each(body, function (value, key) {
      ...
      if (schema) {
        ...
        if (req.query.include) {
          const includes = _.isUndefined(req.scopedIncludes) ? req.query.include : req.scopedIncludes;
   ...
   * @returns {*}
   */
  function findResourcesWithFortuneExtensions (ids, projection, localTracer, req, res) {
    if (ids) {
    ...
    //run before read
    return localTracer('before-read-hooks', () => beforeReadHook({}, req, res))
      .then(function () {
        // get resources by IDs
        const match = {
          $and: [
        ...
        return localTracer('reading-database', () => {
          zipkinTracer.recordBinary('fortune.mongo_query', zipkin.safeStringify(match));
          zipkinTracer.recordBinary('fortune.mongo_projection', zipkin.safeStringify(projection));
      ...
      .then(function (resources) {
        if(resources.length) {
          ...
          const notRestricted = (req.query.fortuneExtensions.length === 0) ||
          ...
          if (req.query.includeDeleted && notRestricted){// the list includes all available docs
            ...
            adapter.findMany(model, { $and: [lessRestrictiveFilter] }, projectionWithDeleted).then(function (resources) {
              if (resources.length) {
                sendError(req, res, (resources.filter(sift({ $and: req.query.fortuneExtensions })).length>0) ? 410 : 403);
              } else {
                sendError(req, res, 404);
              }
            });
