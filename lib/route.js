...
function route(
      ...
      .then(function (object) {
        return localTracer('metadata-providers', () => {
          const includeMeta =
            ...
            _.without(req.query.includeMeta.split(','), 'count', 'true');
  ...
  function createResource(req, res) {
    ...
    function mix(main, linked, linkedModel) {
      ...
      return RSVP.all(
        _.map(_.isArray(main) ? main : [main], function (item) {
          ...
          toCreate.$$createPromise = createLinked(
            linkedModel,
            _.omit(toCreate, 'id'),
          );
    ...
    function createLinked(linkedModel, linkedResource) {
      return createResources(linkedModel, [linkedResource]).then(function (
        resources,
      ) {
          ...
          return localTracer('resolving-cross-resource-query', () =>
            ...
            instrumentor.createTracer(`${tracePrefix} read`, function (query) {
              ...
              if (
                req.query.includeMeta &&
                (_.isBoolean(req.query.includeMeta) ||
                  req.query.includeMeta.split(',').indexOf('count') !== -1 ||
                  req.query.includeMeta.split(',').indexOf('true') !== -1)
              ) {
        ...
        return acc.then(function (prev) {
          return zipkinTracer.scoped(() => {
            ...
            return zipkinTracer.local('writing-database', () => {
              const match = adapter.preupdate(model, id);
              return adapter
                .update(model, { $and: [update.match, match] }, update.update)
