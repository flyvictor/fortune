...
function route(name, model, resources, inflect, querytree, metadataProviders) {
  var _this = this
    ...
    //Looks up linked resources for individual route. :key is an noun
    , linkedResourceRoute = namespace + '/' + collection + '/:id/:key' + suffix
    //Calls custom action set up on resource. :action is a verb
    , actionRoute = namespace + '/' + collection + '/:id/:action' + suffix
 ...
 router.get(collectionRoute, getResources);
  ...
  if (this.options.enableWebsockets){
    ...
    this.io.of(collectionRoute).use(function(socket, next){
      ...
      res.send = function(status, message){
  ...
  function getResources(req, res) {
    ...
    beforeReadHook({}, req, res)
      .then(  instrumentor.createTracer( tracePrefix + ' parse', function(){
        ...
        return querytree.parse(model.modelName, _.extend(match, req.query.filter))
          ...
          if(req.query.includeMeta && (_.isBoolean(req.query.includeMeta) || req.query.includeMeta.split(',').indexOf('count') !== -1 || req.query.includeMeta.split(',').indexOf('true') !== -1)) {
            queries.count = adapter.count(model, {}, projection);
      ...
      .then( instrumentor.createTracer( tracePrefix + ' sending-response', function(r) {
        ...
        if (r.count) {
          ...
          if (r.filterCount) body.meta.filterCount = r.filterCount;
        }
        sendResponse(req, res, 200, body);
      }))
      .catch(function(error) {
        var status = error.constructor.name === "CastError" ? 400 : 500;
   ...
   */
  router.get(linkedResourceRoute, getSubresources);
  ...
  function deleteResource(req, res) {
    ...
    adapter.findMany(model, id, projection)
          ...
          if (operation.op === 'replace') {
            ...
            if (field.length > 1 && field[0] !== "links") {
              ...
              if( !nestedObjects ){
                ...
                path[ pullKey ][ fieldName ] = path[ pullKey ][ fieldName ] || { _id: { $in: [] } };
                path[ '$push' ] = path[ '$push' ] || {};
                path[ '$push' ][ "_internal.deleted." + fieldName ] = path[ '$push' ][ "_internal.deleted." + fieldName ] || { $each: [] };

                var pullObj = path[ pullKey ][ fieldName ]._id;
                var pushObj = path[ '$push' ][ "_internal.deleted." + fieldName ];
