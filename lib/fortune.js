...
var customTypesHelpers = require('./custom-types');
...
Fortune.prototype._init = function (options) {
  ...
  router.use(function(req, res, next){
    req.query = qs.parse(req._parsedUrl.query, {depth: 100});
    req.fortune = {};
    next();
  });
...
Fortune.prototype.resource = function(name, schema, options, schemaCallback) {
  ...
  var customTypesMap = customTypesHelpers.mapCustomTypes(schema, _this._customTypes);
  ...
  var customHooks = hooks.fromCustomTypesMap(customTypesMap);

  var hooks2Merge = _.compact((customHooks || []).concat([_this._resources[name].hooks]));
  ...
  this.adapter.awaitConnection().then(
    instrumentor.createTracer( customInstrumentorObj.options.tracePrefix + ' await connection', () => {
      ...
      _this._route(name, _this.adapter.model(name, schema, modelOptions), _this._resources, inflect, _this._querytree, _this._metadataProviders, _this.zipkinTracer);
...
function checkInternalRefs(resNames, currentName, schema) {
  const internalRefs = _.filter(schema, (prop) => _.isArray(prop) ? prop[0] && prop[0].ref && !prop[0].external : prop.ref && !prop.external)
  ...
  if (refMissing && refMissing.length) {
    console.error(`Some references for resource ${currentName} were not found internally. Missing: ${_.pluck(refMissing, 'ref')}`);
