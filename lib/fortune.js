...
Fortune.prototype._defaults = {
  ...
  customInstrumentorObj: {
    instrumentor: {
      ...
      captureException: function (error, { extra, req, res }) {},
    },
    options: {},
  },
...
Fortune.prototype._init = function (options) {
  ...
  // Setup express.
  if (
    (typeof options.cors === 'boolean' || typeof options.cors === 'object') &&
    options.cors
  ) {
...
Fortune.prototype._exposeResourceDefinitions = function () {
  ...
  // Expose a light version of resources definitions at `/resources/fortune-client`
  const fortuneClientResourcesRoute = '/resources/fortune-client';
  this.router.get(fortuneClientResourcesRoute, function (req, res) {
    if (_this._ready) {
      const resources = _this.resources(req);

      const lightResources = resources.map(function (resource) {
        return {
          name: resource.name,
      ...
      res.write(JSON.stringify({ resources: lightResources }));
      res.end();
    } else {
      res.writeHead(503);
      res.end();
  ...
  // Expose a full version of resources definitions at `/resources` or `/resources/{serviceName}`
...
Fortune.prototype.resource = function (name, schema, options, schemaCallback) {
  ...
  hooks.initGlobalHooks(
    _this._resources[name],
    _this._globalHooks,
    _this.options,
  );
  ...
  this.beforeWrite(function (req) {
    ...
    if (validateErrors.length > 0) {
      for (const validationError of validateErrors) {
  ...
  this.adapter
    ...
    .then(
      instrumentor.createTracer(
        ...
        () => {
          checkInternalRefs(_.keys(_this._resources), name, schema);
    ...
    .catch((e) => {
      ...
      instrumentor.captureException(e, {
        extra: { resource: name },
      });
    });
...
Fortune.prototype.beforeAllRW = function (hooksArray, inlineConfig) {
  this.registerGlobalHook('_before', 'read', hooksArray, inlineConfig);
  this.registerGlobalHook('_before', 'write', hooksArray, inlineConfig);
  return this;
};
Fortune.prototype.afterAllRW = function (hooksArray, inlineConfig) {
  this.registerGlobalHook('_after', 'read', hooksArray, inlineConfig);
  this.registerGlobalHook('_after', 'write', hooksArray, inlineConfig);
  return this;
};
...
Fortune.prototype.registerGlobalHook = function (when, type, provider, config) {
  for (const hook of provider) {
    const inlineConfig = {};

    inlineConfig[hook.name] = matchHookConfig(hook, config);
    hook.priority = hook.priority || 0;

    this._globalHooks[when][type].push({
      constructor: hook,
...
Fortune.prototype._clearGlobalHooks = function () {
  this._globalHooks = {
    _before: {
      read: [],
      write: [],
...
function GlobalHook(time, type) {
  return function (hooksArray, config) {
    this.registerGlobalHook(time, type, hooksArray, config);
    return this;
  };
}
 ...
 * @param {Array} [routes]
 */
Fortune.prototype._removeRoutes = async function (name, methods, routes) {
  const router = this.router;
  const collection = this.options.inflect ? inflect.pluralize(name) : name;
  const re = new RegExp(`\/${collection}`);

  await this.adapter.awaitConnection();

  const routeMatches = function (route) {
    return routes ? _.includes(routes, route.path) : re.test(route.path);
  };

  // Express 3 exposes routes by HTTP verb on `router.routes`.
  // Express 4 stores them in `router._router.stack`, handled below.
  if (router.routes) {
  ...
  if (!_.isArray(stack)) return;

  for (let index = stack.length - 1; index >= 0; index--) {
    const layer = stack[index];
    const route = layer.route;
    if (!route || !routeMatches(route)) continue;

    if (route.methods && route.methods._all) {
      stack.splice(index, 1);
      continue;
    }

    for (const verb of methods || []) {
      if (route.methods && route.methods[verb]) {
        delete route.methods[verb];
      }
    }

    if (_.isArray(route.stack)) {
      route.stack = route.stack.filter(function (routeLayer) {
        return !_.includes(methods, routeLayer.method);
      });
    }

    if (!_.some(route.methods)) {
      stack.splice(index, 1);
    }
  }
};
...
Fortune.prototype._validateResource = function (method, model, resource) {
  ...
  for (const [key, value] of Object.entries(this._resources[model].schema)) {
    if (
 ...
 * @param config
 */
function matchHookConfig(hook, config) {
  if (!hook.name || !config) return hook.config || {};
  const hookName = hook.name;
  const defaultConfig = _.extend({}, hook.config);
  const newConfig = config[hookName] || {};
