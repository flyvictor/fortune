  ...
  return _.reduce(Object.keys(source), function(memo, sourceKey){
    ...
    switch(type){
      case '$pushAll':
        ...
        resultValue.$each = targetValue ? (targetValue.$each || targetValue).concat(sourceValue) : sourceValue;
        break;
      case '$pullAll':
...
exports.buildPatchOperations = function(model, returnedDocument, ops){
  ...
  ops.forEach(function (operation) {
    ...
    preprocessed.forEach(function(meta){
      ...
      first.update[meta.mongoKey || meta.key] = first.update[meta.mongoKey ||meta.key] || {};
      exports.squash(meta.key, first.update[meta.mongoKey || meta.key], meta.update);
    });
  });
