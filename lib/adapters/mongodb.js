...
function deepReplaceKey(data, keysMap) {
  if (Array.isArray(data)) {
    return data.map((item) => deepReplaceKey(item, keysMap));
  }

  if (data && typeof data === 'object') {
    return Object.keys(data).reduce((result, key) => {
      const updatedKey = keysMap[key] || key;
      result[updatedKey] = deepReplaceKey(data[key], keysMap);
      return result;
    }, {});
  }
...
function getInverseReferences(model, modifiedRefs) {
  return getAllReferences(model, modifiedRefs).filter(function (ref) {
    return !!ref.inverse;
  });
}
...
const createAdapter = function () {
  ...
  adapter.schema = function (name, schema, options, schemaCallback) {
    ...
    for (const [key, val] of Object.entries(schema)) {
      ...
      const pkType =
        value.type || value.pkType || mongoose.Schema.Types.ObjectId;
    ...
    if (pk) {
      if (_.isFunction(schema[pk])) {
        ...
        throw new Error(
          'Schema PK must either be a type function or an object with a `type` property',
        );
      }
    ...
    mongooseSchema.schemaCfg = schema;
  ...
  adapter.model = function (name, schema, options) {
    if (schema) {
      ...
      if (options?.shardedModel) {
        ...
        this._models[options.shardedModel.name] = _.extend(
          this.db.model(options.shardedModel.name, newSchema),
          options,
        );
      }
  ...
  adapter.create = async function (model, id, resource) {
    const _this = this;
    if (!resource) {
      resource = id;
    } else {
    ...
    if (upsert.status) {
      ...
      async function tryUpsert(count) {
        ...
        try {
          ...
          if ((error.code === 16837 || error.code === 11000) && count < 5) {
            return tryUpsert(count + 1);
          }
          throw error;
        }
      }

      const created = await tryUpsert(0);
      return _this._handleWrite(model, created);
    }

    const created = await model.create(resource);
    return _this._handleWrite(model, created);
  };
  ...
  adapter.update = async function (model, match, update) {
    const _this = this;
    model = typeof model == 'string' ? this.model(model) : model;
    ...
    try {
      const resource = await model.findOneAndUpdate(match, correctUpdate, {
        new: true,
      });
      if (_.isNull(resource)) return;
      return _this._handleWrite(
        model,
        resource,
        null,
        null,
      ...
      console.log('error reading back updated resource ', match, correctUpdate);
      throw error;
    }
  };

  adapter.markDeleted = async function (model, id) {
    const _this = this;
    model = typeof model == 'string' ? this.model(model) : model;
    const pk = model.pk || '_id';
    ...
    const resources = await model.find(match).exec();
    const markedResources = await Promise.all(
      _.map(resources, async function (resource) {
        ...
        await model.updateMany({ _id: resource._id }, update);
        return model.findOne({ _id: resource._id });
      }),
    );

    return Promise.all(
      _.map(markedResources, async function (resource) {
        await _this._handleWrite(model, resource);
        return _this._deserialize(model, resource);
      }),
    );
  };

  adapter.delete = async function (model, id) {
    const _this = this;
    //Delegating to markDeleted to handle linking
    const returnValue = await _this.markDeleted(model, id);
    model = typeof model == 'string' ? this.model(model) : model;
    const pk = model.pk || '_id';
   ...
   * @returns {Promise}
   */
  adapter.find = async function (model, query, options) {
    if (!_.isObject(query)) query = { id: query };

    options = options || {};
    options.select = removeUselessProjectionSelect(options.select, model);
    options.limit = 1;
    const resources = await adapter.findMany(model, query, options);
  ...
  adapter.findMany = function (model, query, options) {
    return adapter._findMany(model, query, options, false);
  };

  adapter.count = function (model, query, options) {
    return adapter._findMany(model, query, options, true);
  };
  ...
  adapter._findManyModel = async function (model, query, options, count) {
    const _this = this;
    ...
    if (options.page && options.page > 0) {
      options.skip = (options.page - 1) * options.pageSize;
      options.limit = options.pageSize;
    }

    //Ensure business id is included to selection
    let pkNotRequested = false;
    if (_.isArray(options.select)) {
      if (model.pk) {
        if (options.select.indexOf(model.pk) === -1) {
          options.select.push(model.pk);
          pkNotRequested = true;
        }
      }
    ...
    const q = model.find(query).limit(options.limit).select(options.select);
    ...
    const resources = await q.skip(options.skip).lean(true).exec();
  ...
  adapter._findMany = async function (model, query, options, count) {
    model = typeof model == 'string' ? this._models[model] : model;
    ...
    if (
      model.shardedModel &&
      options?.canUseShardedCollection &&
      this._models[model.shardedModel.name]
    ) {
      try {
        const resources = await this._findManyModel(
          this._models[model.shardedModel.name],
          deepReplaceKey(_.clone(query), { _id: 'originalId' }),
          options,
          count,
        );
        resources.forEach((resource) => {
          if (!model.pk && resource.originalId) {
            resource.id = new mongoose.Types.ObjectId(resource.originalId);
            delete resource.originalId;
          }
          delete resource.location;
        });
        return resources;
      } catch (err) {
        console.log('shardedModel error, fallback to default model', err);
      }
    }
    return await this._findManyModel(model, query, options, count);
  };
  ...
  adapter._serialize = function (model, resource) {
    ...
    if (resource.hasOwnProperty('links') && typeof resource.links == 'object') {
      for (const [key, value] of Object.entries(resource.links)) {
        resource[key] = value;
      }
  ...
  adapter._deserialize = function (model, resource) {
    ...
    if (relations.length) {
      ...
      for (const relation of relations) {
        if (
          _.isArray(json[relation]) ? json[relation].length : json[relation]
        ) {
  ...
  adapter._handleWrite = async function (
    ...
    try {
      const [returnedResource] = await Promise.all([
        this._updateRelationships(model, resource, modifiedRefs),
        this._updateShardedCollection(model, resource),
      ]);
      if (!returnedResource) {
        throw new Error(
      ...
      const deserialized = _this._deserialize(model, returnedResource);
      if (resolve) resolve(deserialized);
      return deserialized;
    } catch (error) {
      if (reject) return reject(error);
      throw error;
  ...
  adapter._updateShardedCollection = async function (model, resource) {
    if (!model.shardedModel) return;
    const shardedModel = this._models[model.shardedModel.name];
    const originalId = resource._id.toString();
    await shardedModel.deleteMany({ originalId });
    if (!resource.deletedAt) {
      ...
      await Promise.all(
        model.shardedModel.locations.map((location) => {
          return shardedModel.create({ ...newResource, location });
        }),
      );
    }
  };
   ...
   * @return {Promise}
   */
  adapter._updateRelationships = async function (
    model,
    resource,
    modifiedRefs,
  ) {
    ...
    for (const reference of references) {
      ...
      if (!reference.isExternal) {
        ...
        for (const [key, value] of Object.entries(relatedTree)) {
          const singular = !_.isArray(value);
          const obj = singular ? value : value[0];
  ...
  adapter._updateManyToOne = async function (
    ...
    // Associate NEWs
    const newMatch = { ...initialMatch };
    newMatch[relatedModel.pk || '_id'] = {
      $in: resource[reference.path] || [],
    };
