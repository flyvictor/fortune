...
adapter.schema = function (name, schema, options, schemaCallback) {
  ...
  if (schema.location && schema._tenantId) {
    console.log('configuring for sharding: ', name);
    options.shardKey = { location: 1, _tenantId: 1 };
  }
...
adapter.model = function(name, schema, options) {
  if(schema) {
    ...
    model.isSharded = new Promise(function(resolve, reject){
      _this.db.collection(model.collection.collectionName).stats()
        .then(function(stat){
          resolve(stat.sharded);
        }).catch(reject);
    });
      ...
      function tryUpsert(count){
        return new Promise(function(resolve, reject){
          model.findOne(upsert.match, function(err, matched){
            ...
            _this.getShardQueryExtension(model, upsert.match).then(function(shardQuery) {
              model.findOneAndUpdate({...shardQuery, ...upsert.match}, { $set: getFullKeys(update, matched && matched.toObject()) }, _.extend({}, upsert.opts, { new: true }), function (error, r) {
...
adapter.getShardQueryExtension = async function(model, match, localResource) {
  model = typeof model == 'string' ? this.model(model) : model;

  const [isSharded, record] = await Promise.all([
    model.isSharded,
    localResource || model.findOne(match, {_tenantId: 1, location: 1})
  ]);

  if (isSharded && record) {
    const obj = record.toObject();
    return {
      _tenantId: obj._tenantId,
...
adapter.update = function (model, match, update) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    _this.getShardQueryExtension(model, match).then(function(shardQuery) {
      ...
      model.findOneAndUpdate({...shardQuery, ...match}, correctUpdate, { new: true }, function (error, resource) {
...
adapter._updateOneToOne = function(model, relatedModel, resource, reference, field) {
  ...
  return _this.getShardQueryExtension(relatedModel, match, resource).then(function(shardQuery) {
    const fullQuery = {...shardQuery, ...match};
    return wrapAsyncCall(relatedModel, relatedModel.update, fullQuery, dissociate)
  }).then(function(){
      // Association
      var associate = {$set: {}};
      ...
      return _this.getShardQueryExtension(relatedModel, match, resource).then(function(shardQuery) {
        const fullMatch = {...shardQuery, ...match};
        return wrapAsyncCall(relatedModel, relatedModel.findOneAndUpdate, fullMatch, associate, { new: true });
      });
    });
};
...
adapter._updateOneToMany = function(model, relatedModel, resource, reference, field) {
  ...
  return _this.getShardQueryExtension(relatedModel, match, resource).then(function(shardQuery) {
    ...
    return wrapAsyncCall(relatedModel, relatedModel.update, fullQuery, dissociate)
      ...
      return _this.getShardQueryExtension(relatedModel, match, resource).then(function(shardQuery) {
        const fullQuery = {...shardQuery, ...match};
        return wrapAsyncCall(relatedModel, relatedModel.findOneAndUpdate, fullQuery, associate, { new: true });
      });
    });
};
...
adapter._updateManyToOne = function(model, relatedModel, resource, reference, field) {
  ...
  return _this.getShardQueryExtension(relatedModel, match, resource).then(function(shardQuery) {
    const fullQuery = {...shardQuery, ...match};
    return wrapAsyncCall(relatedModel, relatedModel.update, fullQuery, dissociate, { multi: true })
  }).then(function(){
    // Association
    var associate = {$set: {}};
    associate.$set[field.path] = resource[model.pk || "_id"];
    ...
    return _this.getShardQueryExtension(relatedModel, match, resource).then(function(shardQuery) {
      const fullQuery = {...shardQuery, ...match};
      return wrapAsyncCall(relatedModel, relatedModel.update, fullQuery, associate, { multi: true });
    });
  }).then(function(){
    return unbindRedundant(_this, model, relatedModel, resource, reference.path, field.path);
  });
};



function unbindRedundant(_this, model, relatedModel, resource, refFrom, refTo){
  ...
  return wrapAsyncCall(relatedModel, relatedModel.findOne, match)
    .then(function(matching){
      if (matching){
        ...
        return _this.getShardQueryExtension(model, selfMatch, resource).then(function(shardQuery) {
          const fullQuery = {...shardQuery, ...selfMatch};
          return wrapAsyncCall(model, model.update, fullQuery, unbind, { multi: true });
        });
      }
    });
}
