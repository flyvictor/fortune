...
adapter.schema = function (name, schema, options, schemaCallback) {
  ...
  if (hasTenantId){
    ...
    if (pk){
      var ind = {_tenantId: 1};
      ind[pk] = 1;
      schema.index(ind, {unique: true, sparse: true});
    }
  }
      ...
      function tryUpsert(count){
        return new Promise(function(resolve, reject){
          model.findOne(upsert.match, function(err, matched){
            ...
            model.findOneAndUpdate(upsert.match, {$set: getFullKeys(update, matched && matched.toObject())}, upsert.opts, function(error, r) {
  ...
  _.each(query, function(val, key){
    ...
    if(_.isNull(val) || _.isUndefined(val)){
      ...
      if(_.isString(val)) {
        ...
        query[key] = _.object(_.compact(_.map(["gt", "gte", "lt", "lte"], function(op) {
      ...
      query[key] = _.reduce(val, function(memo, opVal, op){
        memo[{ "gt": "$gt", "gte": "$gte", "lt": "$lt", "lte": "$lte" }[op] || op] = parseFloat(opVal);
        return memo;
      }, {});
...
adapter._findMany = function(model, query, projection, count) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (count) {
      ...
      console.log('find query ', util.inspect(query, {depth: null}));
