...
adapter.update = function (model, match, update) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    model.findOne(match, '_id', function (err, matched) {
      if (err) return reject(err);
      if (_.isNull(matched)) return resolve();

      model.findOneAndUpdate(
        { $and: [{ _id: matched._id }, match] },
        correctUpdate,
        { new: true },
        function (error, resource) {
          if (_.isNull(resource)) return resolve();
          if (error) {
            console.log(match,matched.id, correctUpdate);
          }
          _this._handleWrite(
            model,
            resource,
...
adapter._updateManyToMany = function (
  ...
  return wrapAsyncCall(relatedModel, relatedModel.updateMany, match, dissociate).then(function () {
    // Association
    const associate = { $addToSet: {} };
    associate.$addToSet[field.path] = resource[model.pk || '_id'];
    ...
    return wrapAsyncCall(relatedModel, relatedModel.updateMany, match, associate);
  });
};
