...
const createAdapter = function () {
  ...
  adapter.model = function (name, schema, options) {
    if (schema) {
      ...
      if (options?.shardedModel) {
        const newSchema = this.schema(name, {
          ...schema.schemaCfg,
          location: String,
          originalId: String,
        }, options);
        this._models[options.shardedModel] = _.extend(
          this.db.model(options.shardedModel, newSchema),
          options,
        );
      }
   ...
   * @returns {Promise}
   */
  adapter.find = function (model, query, options) {
    ...
    options.limit = 1;
    return new Promise(function (resolve, reject) {
      return adapter.findMany(model, query, options).then(
        function (resources) {
  ...
  adapter._findManyModel = function (model, query, options, count) {
    const _this = this;
  ...
  adapter._findMany = async function (model, query, options, count) {
    ...
    if (model.shardedModel && options?.canUseShardedCollection) {
      try {
        const resources = await this._findManyModel(
          this._models[model.shardedModel],
  ...
  adapter._handleWrite = async function (
    ...
    try {
      const [_, returnedResource] = await Promise.all([
  ...
  function getInverseReferences(model, modifiedRefs) {
    return adapter
      ._getAllReferences(model, modifiedRefs)
      .filter(function (ref) {
        return !!ref.inverse;
      });
  }
  ...
  adapter._updateShardedCollection = async function (
    model,
    resource,
  ) {
    if (!model.shardedModel || !process.env.SHARDED_MODEL_LOCATIONS) return;
    const shardedModel = this._models[model.shardedModel];
    ...
    if (!resource.deletedAt) {
      ...
      await Promise.all(
        process.env.SHARDED_MODEL_LOCATIONS.split(',').map((location) => {
