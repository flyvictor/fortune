...
adapter.model = function (name, schema, options) {
  if (schema) {
    ...
    let isShardedPromise = null;

    model.isSharded = () => {
      if (isShardedPromise) return isShardedPromise;

      isShardedPromise = new Promise(function (resolve, reject) {
        _this.db
...
adapter.getShardQueryExtension = async function (model, match) {
  model = typeof model == 'string' ? this.model(model) : model;

  const isSharded = await model.isSharded();
  if (!isSharded) return {};

  const record = await model.findOne(match, model.shardKey, { lean: true });
  if (record) {
    return _.pick(record, Object.keys(model.shardKey));
  } else {
    // Nothing matched in a sharded collection. This could happen processing positional updates.
    // There we issue two commands, one for normal flow, another for deleted section. One of them matches nothing.
    // Details on how it's generated are covered in route-helpers.js, "replace" op
    return false;
  }
};
...
adapter.update = function (model, match, update) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this
      .getShardQueryExtension(model, match)
      .then(function (shardQuery) {
        if (shardQuery === false) {
          //Bail out if collection is sharded and no update target matched
          return resolve();
        }

        model.findOneAndUpdate(
          {
            $and: [shardQuery, match],
          },
          correctUpdate,
          { new: true },
...
adapter._updateManyToOne = function (
  ...
  return wrapAsyncCall(relatedModel, relatedModel.updateMany, match, dissociate)
    .then(function () {
      // Association
      const associate = { $set: {} };
      associate.$set[field.path] = resource[model.pk || '_id'];
