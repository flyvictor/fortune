...
function removeUselessProjection (projection, model) {
  const projectionKeys = _.keys(projection);
  const {
    nonNestedProjection,
    nestedProjection,
  } = _.reduce(projectionKeys, function (acc, key) {
    if (_.includes(key, '.')) {
      acc.nestedProjection.push(key);
    } else {
      acc.nonNestedProjection.push(key);
    }
    return acc;
  }, {
    nonNestedProjection: [],
  ...
  if (!nonNestedProjection.length || !nestedProjection.length) {
    return projection;
  }

  const keysToDelete = _.reduce(nonNestedProjection, function (acc, nonNestedKey) {
    const nestedKey = nestedProjection.find((nestedKey) => nestedKey.startsWith(nonNestedKey)
  ...
  keysToDelete.forEach((key) => {
    console.warn(`[fortune::mongodb-adapter] Useless "${key}" projection key required for model ${collectionName}.`);
