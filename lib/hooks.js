...
var customTypeUtils = require('./custom-types');
...
exports.merge = function(hookset, rec) {
  var isArray = _.some(hookset, function(arg) { return _.isArray(arg); });
  if(isArray) {
  ...
  var keys = _.uniq(_.flatten(_.map(hookset, function(arg) {
    return _.keys(arg);
  })));

  return _.fromPairs(_.map(keys, function(key) {
...
exports.fromCustomTypesMap = function(customTypes) {
  return customTypes.map(function(type){
    var result = _.cloneDeep(type.hooks);
        ...
        _.each(actionHooks, function(ahook) {
          ...
          if(handler) {
            ahook.fn = function(req, res) {
              var doc = this;
              return customTypeUtils.applyHook(handler, type.path, doc, req, res)
                .then(function(){
                  return doc;
                });
            }
