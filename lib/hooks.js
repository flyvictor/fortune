  ...
  name.split(' ').forEach(function (resourceName) {
    ...
    if (!_this._resources || !_this._resources[resourceName]) {
      if (!_this._customTypes || !_this._customTypes[resourceName]) {
        return console.warn(
          `You are trying to attach a hook to ${resourceName}, that is not defined in this instance of fortune`,
        );
...
exports.fromCustomTypesMap = function (customTypes) {
  return customTypes.map(function (type) {
    ...
    for (const whenHooks of Object.values(result || {})) {
      for (const actionHooks of Object.values(whenHooks || {})) {
        for (const ahook of actionHooks || []) {
          if (ahook.name) {
