  ...
  describe('fortune-multitenant integration', function () {
    ...
    it('should allow to create duplicate PK for different tenants', function (done) {
      request(baseUrl)
        ...
        .send(
          JSON.stringify({
            people: [{ email: 'dilbert@mailbert.com', _tenantId: 'testing' }],
          }),
        )
        .end(function (err, res) {
          ...
          request(baseUrl)
            ...
            .end(function (err, res) {
              ...
              const matches = _.filter(body.people, function (p) {
                return p.email === 'dilbert@mailbert.com';
              });
        ...
        request(baseUrl)
          ...
          .end(function (err) {
            ...
            request(baseUrl)
              ...
              .send(
                JSON.stringify({
                  pets: [{ name: 'Ratbert', _tenantId: 'testing' }],
                }),
              )
              .expect(201)
              .end(function (err, res) {
      ...
      it('should bind one-to-one references to correct tenant', function (done) {
        request(baseUrl)
          ...
          .end(function (err) {
            should.not.exist(err);
            request(baseUrl)
              .get('/people?filter[email]=dilbert@mailbert.com')
              .end(function (err, res) {
                should.not.exist(err);
                const body = JSON.parse(res.text);
                const regularResoruce = _.find(body.people, function (p) {
                  return !p._tenantId;
                });
                const tenantedResource = _.find(body.people, function (p) {
                  return p._tenantId;
                });
