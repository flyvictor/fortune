...
module.exports = function (options) {
  describe('should upsert logic', function () {
    ...
    function countDupes() {
      return new Promise(function (resolve) {
        request(baseUrl)
          .get('/people?filter[upsertTest]=match')
    ...
    it('should generate a valid _id for upserted record', function () {
      return createDuplicate()
        .then(function () {
          return options.app.adapter.mongoose.connections[
            options.app.adapter.mongoose.connections.length - 1
          ].db
            .collection('people')
            .findOne({ upsertTest: 'match' });
        })
        .then(function (match) {
