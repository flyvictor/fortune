  ...
  describe('compound document support', function () {
    ...
    it('should return immediate child documents of people when requested, ignoring invalid includes', function (done) {
      request(baseUrl)
        ...
        .end(function (error, response) {
          ...
          body.people[0].nickname.should.equal(
            `Super ${fixtures.people[0].name}!`,
          );
          body.linked.people[0].nickname.should.equal(
            `Super ${fixtures.people[1].name}!`,
          );
          done();
        });
    });
      ...
      new Promise(function (resolve) {
        request(baseUrl)
          ...
          .end(function (err, res) {
            ...
            resolve(
              Promise.all(
                _.map(_.tail(body.addresses), function (addr) {
        ...
        request(baseUrl)
          .get(`/people/${ids.people[0]}?include=addresses`)
          .expect(200)
          .end(function (err, res) {
            should.not.exist(err);
            const body = JSON.parse(res.text);
    ...
    it('should not attempt to include resource (to-1) marked as external', function (done) {
      request(baseUrl)
        ...
        .end(function (err, res) {
          should.not.exist(err);
          const body = JSON.parse(res.text);
          body.cars[0].links.should.eql({
            MOT: 'fakeref',
            owner: 'dilbert@mailbert.com',
          });
    ...
    describe('Compound document creation', function () {
      ...
      it('should properly reference linked documents from primary documents', function (done) {
        request(baseUrl)
          .get('/people/dwalin@dwarf.com,balin@dwarf.com')
          .expect(200)
          .end(function (err, res) {
            should.not.exist(err);
            const body = JSON.parse(res.text);
            const dwalin = _.find(body.people, { name: 'Dwalin' });
            ...
            const balin = _.find(body.people, { name: 'Balin' });
      ...
      it('should properly reference primary documents from linked documents', function (done) {
        request(baseUrl)
          .get(`/pets/${pets.Lumpy},${pets.Cuddles},${pets.Toothy}`)
          .expect(200)
          .end(function (err, res) {
            should.not.exist(err);
            const body = JSON.parse(res.text);
            _.find(body.pets, { name: 'Lumpy' }).links.owner.should.equal(
              'dwalin@dwarf.com',
            );
            _.find(body.pets, { name: 'Cuddles' }).links.owner.should.equal(
              'dwalin@dwarf.com',
            );
            _.find(body.pets, { name: 'Toothy' }).links.owner.should.equal(
              'balin@dwarf.com',
            );
            done();
          });
      ...
      it('should be able to properly map bindings between same-type fields', function (done) {
        request(baseUrl)
          ...
          .end(function (err, res) {
            ...
            const shackId = body.people[0].links.houses[0];
            const mansionId = body.people[0].links.estate;
            const shack = _.find(body.linked.houses, { address: 'shack' });
            const mansion = _.find(body.linked.houses, { address: 'mansion' });
      ...
      it('should succeed to link new resource to both existing and new resource', function (done) {
        request(baseUrl)
          ...
          .end(function (err, res) {
            should.not.exist(err);
            const body = JSON.parse(res.text);
            body.people[0].links.houses
              .indexOf(ids.houses[0])
              .should.not.equal(-1);
      ...
      it('should keep predictable order of the hooks ', function (done) {
        request(baseUrl)
          ...
          .end(function (err, res) {
            should.not.exist(err);
            res
              .get('compound-post-hooks-order-person-passed')
              .should.equal('1');
