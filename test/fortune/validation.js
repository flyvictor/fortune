...
module.exports = function (options) {
  ...
  describe('Resource validation', function () {
    ...
    after(function () {
      app._resources['person'].schema['name'] =
        app._resources['person'].schema['name'].type;
    });
    ...
    it('should add validator to the list', function (done) {
      app.addValidator('petAge', {
        numericality: { moreThan: 0, lessThan: 30 },
      });
    ...
    it('should prevent creation of an invalid resource via POST', function (done) {
      request(baseUrl)
        ...
        .send(
          JSON.stringify({
            people: [
              { name: 'Peter', password: 'qwerty', email: 'test@example.com' },
            ],
          }),
        )
        .end(function (error) {
    ...
    it('should allow creation of a valid resource via POST', function (done) {
      request(baseUrl)
        ...
        .send(
          JSON.stringify({
            people: [
              { name: 'Aaron', password: 'qwerty', email: 'test@example.com' },
            ],
          }),
        )
        .end(function (error) {
    ...
    it('should prevent PATCH with an invalid value', function (done) {
      request(baseUrl)
        ...
        .send(
          JSON.stringify([
            { path: '/people/0/name', op: 'replace', value: '' },
          ]),
        )
        .end(function (error) {
          if (error) return done(error);
    ...
    it('should allow PATCH with a valid value', function (done) {
      request(baseUrl)
        ...
        .send(
          JSON.stringify([
            { path: '/people/0/name', op: 'replace', value: 'Chris' },
          ]),
        )
        .end(function (error) {
          if (error) return done(error);
    ...
    it('should receive an error response in correct format when POSTing', function (done) {
      request(baseUrl)
        ...
        .send(
          JSON.stringify({
            people: [
              { name: '', password: 'qwerty', email: 'test@example.com' },
            ],
          }),
        )
        .end(function (error, response) {
          response.body.should.have.keys('error', 'detail');
          response.body.detail.should.equal(
            "Error: Validation error: Name can't be blank",
          );
          done();
        });
    });
    ...
    it('should receive an error response in correct format when PATCHing', function (done) {
      request(baseUrl)
        ...
        .send(
          JSON.stringify([
            { path: '/people/0/name', op: 'replace', value: 'Peter' },
          ]),
        )
        .end(function (error, response) {
          response.body.should.have.keys('error', 'detail');
          response.body.detail.should.equal(
            "Error: Validation error: Name 'Peter' is not allowed",
          );
          done();
        });
    });
