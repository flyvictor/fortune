...
module.exports = function (options) {
  ...
  describe('associations', function () {
      ...
      it('should be able to dissociate', function (done) {
        new Promise(function (resolve) {
          request(baseUrl)
            ...
            .send([{ path: '/people/0/links/pets', op: 'replace', value: [] }])
            .expect('Content-Type', /json/)
            .expect(200)
            .end(function (error, response) {
              should.not.exist(error);
      ...
      it('should be able to dissociate', function (done) {
        new Promise(function (resolve) {
          request(baseUrl)
            ...
            .send([{ path: '/pets/0/links/owner', op: 'replace', value: null }])
            .expect('Content-Type', /json/)
            .expect(200)
            .end(function (error, response) {
              should.not.exist(error);
        ...
        new Promise(function (resolve) {
          ...
          request(baseUrl)
            ...
            .end(function (error, response) {
              ...
              const body = JSON.parse(response.text);
              should.not.exist(
                body.people[0].links && body.people[0].links.soulmate,
              );
              done();
            });
        });
        ...
        new Promise(function (resolve) {
          ...
          request(baseUrl)
            ...
            .end(function (error, response) {
              ...
              const body = JSON.parse(response.text);
              should.not.exist(
                body.people[0].links && body.people[0].links.lovers,
              );
              done();
            });
        });
    ...
    describe('many to one', function () {
      it('should unbind other "many" refs when updated', function (done) {
        new Promise(function (resolve) {
          ...
          .then(function () {
            request(baseUrl)
              .get(`/people/${ids.people[0]},${ids.people[1]}`)
              .expect(200)
              .end(function (err, res) {
                should.not.exist(err);
                const body = JSON.parse(res.text);
                const first = _.find(body.people, { id: ids.people[0] });
                const second = _.find(body.people, { id: ids.people[1] });
    ...
    it('should be indexed', function (done) {
      ...
      (model = app.adapter.model('person')).collection.getIndexes(function (
        err,
        indexData,
      ) {
        for (const key of model.schema.refkeys) {
