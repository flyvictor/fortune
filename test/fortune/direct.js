...
module.exports = function (options) {
  describe('direct', function () {
    ...
    it('should be compatible with fortuneClient including metadata', function (done) {
      app.direct
        .get('people', { query: { includeMeta: true } })
        .then(function (res) {
          ids.people.forEach(function (id) {
            _.includes(_.map(res.body.people, 'id'), id).should.equal(true);
          });
    ...
    it('gets a single resource', function () {
      return app.direct
        .get('people', { params: { id: ids.people[0] } })
        .then(function (res) {
    ...
    it('creates a resource', function (done) {
      const doc = { people: [{ name: 'Director', email: 'director@abc.com' }] };

      app.direct
        .create('people', { body: doc })
        .then(function (res) {
          res.body.people.length.should.be.equal(1);
          res.body.people[0].id.should.be.equal(doc.people[0].email);
          return app.direct.get('people', {
            params: { id: doc.people[0].email },
          });
        })
        .then(function (res) {
          res.body.people[0].id.should.be.equal(doc.people[0].email);
    ...
    it('supports filters', function (done) {
      app.direct
        .get('people', { query: { filter: { name: 'Robert' } } })
        .then(function (res) {
